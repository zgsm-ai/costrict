...
export default function CloudProductionView({
	...
	const selectedConversationSummary = selectedConversationId
		? conversationSummaries.get(selectedConversationId)
		: undefined
							...
							<div className="mx-auto flex w-full max-w-5xl items-center justify-between gap-3">
								<div className="flex min-w-0 items-center gap-2">
									...
									<div className="items-center gap-1.5 sm:flex">
										...
										{selectedConversationSummary ? (
											<ConversationTone
												tone={selectedConversationSummary.tone}
												label={selectedConversationSummary.label}
											/>
										) : null}
									</div>
								</div>
								<div className="flex flex-wrap items-center justify-end gap-2">
									...
									<Button
										variant="outline"
										size="sm"
										className="rounded-full border-vscode-panel-border/50 bg-transparent"
						...
						<div className="min-h-0 flex-1 pl-3 pr-0 py-0">
							<div className="mx-auto flex h-full w-full max-w-5xl flex-col gap-3">
								...
								{!ready ? (
									<EmptyState
										title="Cloud 尚未就绪"
										description="请先确认认证、server 地址和健康检查均已通过。"
									/>
								) : !selectedConversationId ? (
									<EmptyState
										title="请选择会话"
										description="先新建一个会话，或从左上角会话抽屉切换历史会话。"
									/>
								) : messages.length === 0 ? (
									<EmptyState
										title="暂无消息"
										description="当前会话还没有消息，可以发送一条 prompt，或手动读取 Messages。"
									/>
								) : (
			...
			{showConversationDrawer ? (
				<>
					<button
						type="button"
						aria-label="关闭会话抽屉"
						className="absolute inset-0 z-30 bg-[color-mix(in_srgb,var(--vscode-editor-background)_42%,transparent)] backdrop-blur-[1px]"
						...
						<div className="border-b border-vscode-panel-border/70 px-4 py-4">
							<div className="flex items-start justify-between gap-3">
								<div>
									...
									<div className="mt-1 text-base font-semibold text-vscode-sideBar-foreground">
										历史会话
									</div>
						...
						<div className="border-b border-vscode-panel-border/70 px-4 py-3">
							<div className="relative">
								...
								<Input
									...
									placeholder="搜索标题、ID、状态"
									className="rounded-xl border-vscode-panel-border/70 bg-vscode-input-background py-2 pl-9 pr-9 text-sm text-vscode-input-foreground"
								/>
								{conversationQuery ? (
									<button
										type="button"
										className="absolute right-3 top-1/2 -translate-y-1/2 text-vscode-descriptionForeground hover:text-vscode-editor-foreground"
							...
							<div className="mt-3 flex items-center justify-between gap-2">
								...
								<div className="rounded-full border border-vscode-panel-border/70 bg-vscode-editor-background/45 px-2.5 py-1 text-[11px] text-vscode-descriptionForeground">
						...
						<div className="scrollbar-cloud min-h-0 flex-1 overflow-y-auto px-4 py-4">
							<div className="space-y-2.5">
								{filteredConversations.length === 0 ? (
									<div className="rounded-2xl border border-dashed border-vscode-panel-border/80 bg-vscode-editor-background/35 p-5 text-sm text-vscode-descriptionForeground">
										{conversationQuery ? "没有匹配的会话" : "暂无会话"}
									</div>
								) : (
											...
											<button
												...
												<div className="flex items-start justify-between gap-3">
													<div className="min-w-0 flex-1">
														<div className="truncate text-sm font-medium text-vscode-sideBar-foreground">
															{conversation.title || id}
														</div>
														<div className="mt-1 line-clamp-2 text-xs text-vscode-descriptionForeground">
															{summary?.detail || conversation.status || "空闲"}
														</div>
														<div className="mt-2 flex flex-wrap gap-1.5 text-[11px] text-vscode-descriptionForeground">
															<span className="rounded-full border border-vscode-panel-border/70 bg-vscode-editor-background/42 px-2 py-0.5">
	...
	return (
		<span
			className={cn(
				...
				tone === "running" && "bg-vscode-testing-iconPassed/15 text-vscode-testing-iconPassed",
				tone === "waiting" && "bg-vscode-testing-iconQueued/15 text-vscode-testing-iconQueued",
				tone === "error" && "bg-vscode-errorForeground/15 text-vscode-errorForeground",
				tone === "idle" && "bg-vscode-badge-background text-vscode-badge-foreground",
			)}>
			{label}
		</span>
	)
	...
	return (
		<ToolUseBlock className="rounded-[20px] border border-vscode-panel-border/75 bg-vscode-editor-background/55 shadow-[0_10px_30px_rgba(0,0,0,0.08)]">
			<ToolUseBlockHeader className="gap-2 text-sm">
				<ShieldAlert className="size-4 text-vscode-editorWarning-foreground" />
				<span>{interaction.title}</span>
			</ToolUseBlockHeader>
			<p className="mb-3 whitespace-pre-wrap text-sm leading-6 text-vscode-descriptionForeground">
			...
			<div className="flex flex-wrap gap-2">
				{interaction.options.map((option) => (
					<Button
						key={option.id}
						variant={option.kind?.startsWith("reject") ? "outline" : undefined}
						size="sm"
						className="rounded-full"
						disabled={disabled}
...
function CloudQuestionCard({
	...
	return (
		<ToolUseBlock className="rounded-[20px] border border-vscode-panel-border/75 bg-vscode-editor-background/55 shadow-[0_10px_30px_rgba(0,0,0,0.08)]">
			<ToolUseBlockHeader className="gap-2 text-sm">
				<HelpCircle className="size-4 text-vscode-testing-iconQueued" />
				<span>{interaction.title}</span>
			</ToolUseBlockHeader>
			<p className="mb-3 whitespace-pre-wrap text-sm leading-6 text-vscode-descriptionForeground">
				{interaction.description}
			...
			<Textarea
				...
				disabled={disabled}
				className="min-h-[92px] rounded-2xl border-vscode-panel-border/70 bg-vscode-editor-background/55"
			/>
			<div className="mt-3 flex flex-wrap gap-2">
				<Button size="sm" className="rounded-full" disabled={disabled} onClick={onReply}>
					<SendHorizonal className="size-4" />
					提交回复
				</Button>
				<Button variant="outline" size="sm" className="rounded-full" disabled={disabled} onClick={onReject}>
