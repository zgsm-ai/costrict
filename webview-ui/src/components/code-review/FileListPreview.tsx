...
const FileListPreview = ({ files, isLoading }: FileListPreviewProps) => {
	...
	if (isLoading) {
		return (
			<div className="flex items-center justify-center py-8 text-vscode-descriptionForeground mb-4">
		...
		<div className="bg-vscode-sideBar-background border border-vscode-panel-border rounded-md overflow-hidden mb-4">
			<div className="max-h-[400px] overflow-y-auto">
				{files.map((file, index) => (
					<FileStatusItem key={`${file.path}-${index}`} path={file.path} status={file.status} />
				))}
			</div>
		</div>
	)
