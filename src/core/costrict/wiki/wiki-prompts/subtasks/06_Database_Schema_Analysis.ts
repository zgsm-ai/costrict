...
5. **记录**：确保每个表结构信息都有精确行号引用，使用多文件引用避免清一色单个文件
...
4. **推断**：基于查询代码推断索引使用场景
5. **记录**：为每个索引特征标注精确行号引用，确保引用文件多样性
...
3. **识别**：一对一、一对多、多对多关系
4. **推断**：基于业务逻辑推断关系模型
5. **记录**：为每个关系特征标注精确行号引用，确保引用文件多样性
...
4. **验证**：通过代码确认约束的实现方式
5. **记录**：为每个完整性特征标注精确行号引用，确保引用格式规范
...
${COMMON_OUTPUT_FILE_NAMING(workspace, SUBTASK_OUTPUT_FILENAMES.DATABASE_SCHEMA_TASK_FILE)}
`
