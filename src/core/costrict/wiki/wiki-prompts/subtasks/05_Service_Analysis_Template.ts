...
5. **记录**：确保每个服务功能信息都有精确行号引用，使用多文件引用避免清一色单个文件
...
5. **记录**：为每个接口特征标注精确行号引用，确保引用文件多样性
...
4. **推断**：基于代码结构推导服务依赖图
5. **记录**：为每个依赖关系标注精确行号引用，确保引用文件多样性
...
3. **识别**：响应时间、吞吐量、资源使用率
4. **评估**：基于监控数据分析性能瓶颈
5. **记录**：为每个性能指标标注精确行号引用，确保引用文件多样性
...
${COMMON_SELF_CHECK_BASE}
11. **服务分析特点**：是否针对服务分析模板特点，重点分析了服务功能、接口定义、业务逻辑等？
12. **类图和序列图**：是否包含了服务类图和序列图，并符合规范要求？
...
${COMMON_OUTPUT_FILE_NAMING(workspace, SUBTASK_OUTPUT_FILENAMES.SERVICE_ANALYSIS_TASK_FILE)}
`
