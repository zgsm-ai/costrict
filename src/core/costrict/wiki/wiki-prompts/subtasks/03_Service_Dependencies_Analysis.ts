...
## 核心指导原则（思维链）
1. **证据驱动**：每个依赖结论必须基于具体的文件证据，使用精确行号引用机制标注来源
2. **优先级明确**：按照"80/20法则"，优先提取对代码生成影响最大的依赖信息
...
5. **记录**：使用精确行号引用格式记录所有信息来源，确保引用格式为\`[filename.ext:start_line-end_line](文件路径)\`
...
5. **验证**：对所有相关文件和目录进行验证，确保来源真实性
   ...
   - **目录验证**：对所有目录来源使用list_files工具列出目录内容，确认目录存在
   - **混合验证**：对于不确定是文件还是目录的来源，先尝试文件验证，失败后尝试目录验证
6. **记录**：确保每个服务调用关系都有精确行号引用，使用多文件引用避免清一色单个文件
...
2. **源文件选择**：优先选择对服务依赖分析最重要的文件，如配置文件、服务接口文件、部署文件等
...
### 第九步：自我反思检查清单（质量保证）
1. **信息准确性**：所有依赖信息是否基于实际代码和配置文件？
2. **来源标注**：每个依赖结论是否都有明确的文件路径作为支撑？
3. **来源格式**：来源是否使用\`[filename.ext:start_line-end_line](文件路径)\`格式正确呈现？
4. **多源标注**：是否为关键依赖信息提供了多个来源文件/目录，避免清一色单个文件引用？
