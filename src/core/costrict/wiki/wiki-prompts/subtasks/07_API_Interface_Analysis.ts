...
## 核心指导原则（思维链）
1. **证据驱动**：每个API结论必须基于具体的文件证据，使用精确行号引用机制标注来源
2. **优先级明确**：按照"80/20法则"，优先提取对代码生成影响最大的API信息
...
5. **验证**：对所有相关文件和目录进行验证，确保来源真实性
   ...
   - **目录验证**：对所有目录来源使用list_files工具列出目录内容，确认目录存在
   - **混合验证**：对于不确定是文件还是目录的来源，先尝试文件验证，失败后尝试目录验证
6. **记录**：确保每个API分类信息都有精确行号引用，使用多文件引用避免清一色单个文件
...
5. **验证**：对所有相关文件和目录进行验证，确保来源真实性
   ...
   - **目录验证**：对所有目录来源使用list_files工具列出目录内容，确认目录存在
   - **混合验证**：对于不确定是文件还是目录的来源，先尝试文件验证，失败后尝试目录验证
6. **记录**：确保每个API接口信息都有精确行号引用，使用多文件引用确保来源多样性
...
5. **验证**：对所有相关文件和目录进行验证，确保来源真实性
   ...
   - **目录验证**：对所有目录来源使用list_files工具列出目录内容，确认目录存在
   - **混合验证**：对于不确定是文件还是目录的来源，先尝试文件验证，失败后尝试目录验证
6. **记录**：为每个数据模型特征标注精确行号引用，确保引用文件多样性
...
5. **验证**：对所有相关文件和目录进行验证，确保来源真实性
   ...
   - **目录验证**：对所有目录来源使用list_files工具列出目录内容，确认目录存在
   - **混合验证**：对于不确定是文件还是目录的来源，先尝试文件验证，失败后尝试目录验证
6. **记录**：为每个错误处理特征标注精确行号引用，确保引用格式规范
...
2. **源文件选择**：优先选择对API接口分析最重要的文件，如API定义文件、路由文件、控制器文件等
3. **数量要求**：确保列出至少5个不同的源文件，不足时需主动查找其他相关文件
4. **格式规范**：使用标准markdown链接格式\`- [filename.ext:start_line-end_line](文件路径)\`列出源文件
5. **验证确认**：在生成文档前，再次确认所有列出的源文件都已通过验证
6. **多样性要求**：确保源文件列表包含不同类型的文件，避免单一类型文件集中

### 第十一步：自我反思检查清单（质量保证）
1. **信息准确性**：所有API信息是否基于实际代码和配置文件？
2. **来源标注**：每个API结论是否都有明确的文件路径作为支撑？
3. **来源格式**：来源是否使用\`[filename.ext:start_line-end_line](文件路径)\`格式正确呈现？
4. **多源标注**：是否为关键API信息提供了多个来源文件/目录，避免清一色单个文件引用？
