...
5. **记录**：确保每个测试命令信息都有精确行号引用，使用多文件引用避免清一色单个文件
...
4. **推断**：基于目录结构推断测试组织策略
5. **记录**：为每个测试组织结构特征标注精确行号引用，确保引用文件多样性
...
4. **验证**：通过配置文件确认覆盖率策略
5. **记录**：为每个覆盖率特征标注精确行号引用，确保引用格式规范
...
5. **记录**：为每个测试数据管理特征标注精确行号引用，确保引用文件多样性
...
4. **验证**：通过配置文件确认开发环境设置
5. **记录**：为每个开发环境特征标注精确行号引用，确保引用格式规范
...
${COMMON_OUTPUT_FILE_NAMING(workspace, SUBTASK_OUTPUT_FILENAMES.DEVELOPMENT_TEST_ANALYSIS_TASK_FILE)}
`
