...
- **代码仓库**: 仅读取项目核心架构文件、主要业务逻辑文件和关键配置文件（跳过.roo目录），避免读取测试文件、临时文件和非核心功能代码
...
**基于项目规模的总体规则数量**：
- **小型项目**（技术文档显示简单架构）：10-30条规则
- **中型项目**（技术文档显示中等复杂度）：20-60条规则
- **大型项目**（技术文档显示复杂架构）：40-90条规则

**各维度规则数量分布**：
- **🔴 关键维度**（依赖管理、代码组织、错误处理、测试）：5-10条规则/维度
- **🟡 重要维度**（数据库、API、安全、性能等）：2-6条规则/维度
- **🟢 建议维度**（配置、部署、监控等）：1-4条规则/维度
...
### 内容质量标准
- **项目特异性**: 必须是项目特有的实施细节，而非通用建议
- **技术相关性**: 规则必须与项目实际使用的技术栈和架构相关
- **可验证性**: 规则应能通过代码审查或自动化工具进行验证
...
### 规则生成原则
- **项目特异性优先**: 基于技术文档中的具体信息生成规则，避免通用建议
- **破坏性约束重点**: 优先生成违反后会导致系统错误、架构破坏的约束
- **可验证性**: 每条规则都应该是具体的、可操作的、可验证的
- **简洁有效**: 控制规则数量，确保每条规则都有实际价值

### 严格禁止事项

**绝对禁止在输出文档中包含以下内容**：
- ❌ 禁止添加"规则执行机制"、"违规处理机制"、"规则更新机制"等额外章节
- ❌ 禁止使用除🔴🟡🟢之外的任何优先级标记（如🔥⚡📋等）
- ❌ 禁止添加任何解释性文字、前言、后记或总结
- ❌ 禁止添加任何代码片段、代码示例或伪代码
- ❌ 禁止添加任何通用的编程建议或最佳实践说明
- ❌ 禁止添加任何无法验证的抽象描述
