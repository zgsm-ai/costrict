...
5. **记录**：确保每个集成模式信息都有精确行号引用，使用多文件引用确保来源多样性
...
5. **记录**：为每个格式和协议标注精确行号引用，确保引用格式规范
...
5. **记录**：为每个一致性机制标注精确行号引用，使用多文件引用提供更全面的证据
...
5. **记录**：为每个安全措施标注精确行号引用，确保引用文件多样性
...
${COMMON_OUTPUT_FILE_NAMING(workspace, SUBTASK_OUTPUT_FILENAMES.DATA_FLOW_INTEGRATION_TASK_FILE)}
`
