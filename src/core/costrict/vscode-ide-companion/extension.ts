...
export async function costrictCliActivate(context: vscode.ExtensionContext) {
	...
	const isManagedExtensionSurface = MANAGED_EXTENSION_SURFACES.has(detectIdeFromEnv().name)
	...
	if (!context.globalState.get(INFO_MESSAGE_SHOWN_KEY) && !isManagedExtensionSurface) {
		void vscode.window.showInformationMessage("Gemini CLI Companion extension successfully installed.")
...
export async function costrictCliDeactivate(): Promise<void> {
