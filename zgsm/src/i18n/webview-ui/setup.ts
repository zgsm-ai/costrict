...
const loadTranslationsFromDir = (dirPath: string): LanguageResources => {
	console.log(`Loading zgsm webview-ui translations from ${dirPath}`)
...
// Load zgsm webview-ui translations
export const zgsmTranslations = loadTranslationsFromDir(path.join(__dirname, "locales"))
console.log("Dynamically loaded zgsm webview-ui translations:", Object.keys(zgsmTranslations))
...
export function loadTranslations() {
	Object.entries(zgsmTranslations).forEach(([lang, namespaces]) => {
		try {
			Object.entries(namespaces as Record<string, unknown>).forEach(([namespace, resources]) => {
