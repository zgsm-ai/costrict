...
export const INDEX_GENERATION_AGENT_TEMPLATE: SubtaskTemplateFactory = (workspace: string) => `# 索引文档生成
...
## 🎯 任务目标
为 ${WIKI_OUTPUT_FILE_PATHS.WIKI_OUTPUT_DIR} 文件夹下的技术文档生成结构化索引文件，便于AI快速导航和信息定位。

## 📥 输入要求
- **技术文档目录**: ${WIKI_OUTPUT_FILE_PATHS.WIKI_OUTPUT_DIR} 文件夹下的所有.md技术文档
...
### 🔴 强制约束条件（必须严格遵守）
1. **文档链接路径**: 必须使用 \`${workspace}${WIKI_OUTPUT_FILE_PATHS.WIKI_OUTPUT_DIR}\` 作为父路径前缀，格式为：\`${workspace}${WIKI_OUTPUT_FILE_PATHS.WIKI_OUTPUT_DIR}{文件名}\`
...
## ⚠️ 严格禁止事项
1. ❌ 禁止使用 ./ 或 ../ 等相对路径前缀，必须使用 \`${workspace}${WIKI_OUTPUT_FILE_PATHS.WIKI_OUTPUT_DIR}\` 作为路径前缀
